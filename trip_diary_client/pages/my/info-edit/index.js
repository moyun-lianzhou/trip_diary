...
Page({
    ...
    getUserInfo() {
        ...
        request('/user/info', 'GET', { userId }).then((res) => {
            console.log(res)
            this.setData({userInfo: res.data.userInfo});
            this.setData({photoList:[{
                    url: res.data.userInfo.avatarUrl,
                    name: 'avatar.png',
                    type: 'image',
                    removeBtn: true,
            }]})
    ...
    onSaveInfo() {
        ...
        const userId = wx.getStorageSync('userId')
        const userInfo = { ...this.data.userInfo, userId }; // 确保包含 
        // 没有上传头像
    ...
    onUpload(file, userInfo) {
        const { photoList } = this.data;
        this.setData({photoList: [...photoList, { ...file, status: 'loading' }]});
